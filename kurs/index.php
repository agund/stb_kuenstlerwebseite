<?php
	...
	$html .='<button class="btn btn-secondary" onclick="location.href=\'profil.php?u='.$u.'\'">Profil</button>';
	...
	for ($i=0; $i < count($res); $i++) { 
		if($res[$i]['verkauft_am'] != '0000-00-00 00:00:00' && $res[$i]['verkauft_am'] != NULL) {
			if(substr($res[$i]['verkauft_am'],0,4) == date("Y"))
			{
				$gewinne = $gewinne+$res[$i]['gewinn'];
			}	
		}
	}
	...
	$html .='<div style="float:right;font-weight:bold;margin-right:1em;"><span id="gewinne" style="color:'.($gewinne>=0?'green':'red').';">'.$gewinne.'</span> </div><div style="float:right;font-weight:bold;margin-right:1em;">Gewinne in diesem Jahr: </div>';
	$html .='<div style="float:right;font-weight:bold;margin-right:1em;"><span id="freibetrag" >'.(($steuerfrei - $gewinne)>=0?$steuerfrei - $gewinne:0).'</span> </div><div style="float:right;font-weight:bold;margin-right:1em;">Steuerfrei Gewinn übrig: </div>';
	...
	$html .='<span id="gewinn_'.$i.'" style="color:'.($res[$i]['gewinn']>=0?'green':'red').';">'.$res[$i]['gewinn'].'</span>';
	$html .='</td>';
	$html .='<td>';
	if( $res[$i]['verkauft_am'] == '0000-00-00 00:00:00' || $res[$i]['verkauft_am'] == NULL)
